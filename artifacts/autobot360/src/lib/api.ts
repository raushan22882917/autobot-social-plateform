...
export const apiClient = {
  ...
  me: (token: string) =>
    api<{ user: User; tenant?: { storeName?: string; tenantId?: string } }>('/auth/me', { token }),
  ...
  getSocialStatus: (token: string) =>
    api<{ platforms: Record<string, { configured: boolean; redirectUri: string }> }>('/social/status', { token }),
