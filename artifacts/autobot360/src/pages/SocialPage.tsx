...
function SocialContent() {
  ...
  const [platformStatus, setPlatformStatus] = useState<Record<string, { configured: boolean }>>({});
    ...
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-6">
      ...
      <div className="grid gap-4 md:grid-cols-2">
        {PLATFORMS.map((platform) => {
          ...
          const configured = platformStatus[platform.id]?.configured !== false;
                ...
                <div className="flex-1">
                  ...
                  {connected ? (
                    ...
                    <p className="text-sm text-muted-foreground">
                      {configured ? 'Not connected' : 'Add API keys in services/api/.env'}
                    </p>
                  )}
                </div>
                {connected ? (
