...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  const [authMode] = useState<'firebase' | 'dev'>(() =>
  ...
  const refreshUser = useCallback(async () => {
    ...
    const { user: fresh } = await apiClient.me(storedToken);
  ...
  const login = useCallback(
    ...
    [authMode, exchangeFirebaseToken, persist, setLocation]
  );
  ...
  const signup = useCallback(
    async (data: { email: string; password: string; displayName?: string; storeName: string }) => {
      ...
      if (auth && authMode === 'firebase') {
        ...
        sessionStorage.removeItem(SIGNUP_EXTRAS_KEY);
        await exchangeFirebaseToken(cred.user, extras);
      } else {
  ...
  const loginWithGoogle = useCallback(
    ...
    [exchangeFirebaseToken, setLocation]
  );
