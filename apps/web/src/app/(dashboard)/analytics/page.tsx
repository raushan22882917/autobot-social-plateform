...
export default function AnalyticsPage() {
  ...
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-6">
      <PageHeader
        title="Analytics"
        description="Overview of store performance and engagement."
      />
      ...
      <DataToolbar
        ...
        extra={
          <span className="text-xs text-muted-foreground">
            {loading ? 'Loading…' : `Period: ${PERIODS.find((p) => p.value === period)?.label}`}
          </span>
        }
      />
        ...
        {metrics.map((k) => (
          <motion.div key={k} layout className="glass-card p-5">
            ...
            {!loading && data?.[k]?.change !== undefined && (
              <p
                className={`mt-1 text-xs ${data[k].change >= 0 ? 'text-emerald-400' : 'text-red-400'}`}
              >
                {data[k].change >= 0 ? '+' : ''}
                {data[k].change}% vs prior period
              </p>
            )}
          </motion.div>
        ))}
