...
import { signInWithFirebasePassword } from '../../lib/firebase-password-auth';
...
authRouter.post('/login', async (req, res, next) => {
  try {
    ...
    if (!user.passwordHash) {
      try {
        const idToken = await signInWithFirebasePassword(email, password);
        const decoded = await verifyFirebaseIdToken(idToken);
        const result = await provisionUserFromFirebase(decoded);
        return res.status(200).json(result);
      } catch (err) {
        const message = err instanceof Error ? err.message : 'Firebase sign-in failed';
        if (message.includes('FIREBASE_WEB_API_KEY')) {
          return res.status(503).json({
            error: { message, code: 'FIREBASE_NOT_CONFIGURED' },
          });
        }
        ...
        return res.status(400).json({
          error: {
            message:
              'This account uses Firebase Auth. Add FIREBASE_WEB_API_KEY to services/api/.env, or configure NEXT_PUBLIC_FIREBASE_* in apps/web/.env.local.',
...
authRouter.get('/me', authMiddleware, async (req: AuthRequest, res, next) => {
  try {
    ...
    const plan = (sub?.plan as string) || req.user!.plan || 'free';
    const token = signToken({
      uid: safeUser.uid,
      tenantId: safeUser.tenantId,
      role: safeUser.role,
    ...
    res.json({
      user: safeUser,
      tenant: await db.get('tenants', safeUser.tenantId),
      token,
      expiresIn: 86400,
    });
  } catch (err) {
...
authRouter.post('/refresh', authMiddleware, async (req: AuthRequest, res, next) => {
  try {
    const user = await db.get('users', req.user!.uid);
    if (!user) return res.status(404).json({ error: { message: 'User not found' } });
    const safeUser = sanitizeUserForClient(user as StoredUser);
