...
teamRouter.post('/invite', authMiddleware, requireRole('owner', 'admin'), async (req: AuthRequest, res, next) => {
  try {
...
teamRouter.get('/invites', authMiddleware, requireRole('owner', 'admin'), async (req: AuthRequest, res, next) => {
  try {
    const tenantId = req.user!.tenantId;
...
teamRouter.delete('/invites/:inviteId', authMiddleware, requireRole('owner', 'admin'), async (req: AuthRequest, res, next) => {
  try {
    const { inviteId } = req.params;
    const tenantId = req.user!.tenantId;
...
teamRouter.put('/members/:userId/role', authMiddleware, requireRole('owner', 'admin'), async (req: AuthRequest, res, next) => {
  try {
...
teamRouter.delete('/members/:userId', authMiddleware, requireRole('owner', 'admin'), async (req: AuthRequest, res, next) => {
  try {
    const { userId } = req.params;
    const tenantId = req.user!.tenantId;
